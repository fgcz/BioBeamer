...
def test_get_config_file_name():
    assert biobeamer2.get_config_file_name('file:///path/to/BioBeamer2.xml') == 'BioBeamer2'
...
def test_copy_files_with_tool_robocopy(monkeypatch):
    ...
    monkeypatch.setattr(biobeamer2, 'copy_with_robocopy', lambda src, dst, **kwargs: True)
    # Patch copy_with_scp to ensure it's not called
    monkeypatch.setattr(biobeamer2, 'copy_with_scp', lambda *a, **k: False)
    result = biobeamer2.copy_files_with_tool(source_results, mov, logger, logfile, tool, simulate)
    assert result == [True]
...
def test_copy_files_with_tool_scp(monkeypatch):
    ...
    monkeypatch.setattr(biobeamer2, 'copy_with_scp', lambda src, dst, **kwargs: 'copied')
    result = biobeamer2.copy_files_with_tool(source_results, mov, logger, logfile, tool, simulate)
...
def test_copy_files_with_tool_invalid_tool(monkeypatch):
    ...
    with pytest.raises(Exception):
        biobeamer2.copy_files_with_tool(source_results, mov, logger, logfile, tool, simulate)
...
def test_copy_files(monkeypatch):
    ...
    # Patch get_all_files
    monkeypatch.setattr(biobeamer2, 'get_all_files', lambda p, logger=None: ['a.txt', 'b.txt'])
    # Patch read_copied_files
    monkeypatch.setattr(biobeamer2, 'read_copied_files', lambda storage: ['b.txt'])
    # Patch filter_input_filelist
    monkeypatch.setattr(biobeamer2, 'filter_input_filelist', lambda files, regex, params, logger=None: files)
    # Patch make_destination_files
    monkeypatch.setattr(biobeamer2, 'make_destination_files', lambda files, src, dst: {f: f"{dst}/{f}" for f in files})
    # Patch compare_files_destination
    monkeypatch.setattr(biobeamer2, 'compare_files_destination', lambda mapping: {'copied': {}, 'not_copied': mapping})
    ...
    monkeypatch.setattr(biobeamer2, 'log_copied_files', lambda files, storage: None)
