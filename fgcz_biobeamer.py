...
class BioBeamer(object):
    ...
    def __init__(self, pattern=None, log_file="C:/Progra~1/BioBeamer/fgcz_biobeamer.log", source_path="D:/Data2San/", target_path="\\\\130.60.81.21\\Data2San"):
        ...
        hdlr_syslog21 = logging.handlers.SysLogHandler(address=('130.60.81.148', 514))

        formatter = logging.Formatter('%(asctime)s %(levelname)s %(name)s %(message)s', datefmt="%Y-%m-%d %H:%M:%S")

        hdlr.setFormatter(formatter)
        hdlr_syslog21.setFormatter(formatter)
        self.logger.addHandler(hdlr)
        self.logger.addHandler(hdlr_syslog21)
...
if __name__ == "__main__":
    if str(socket.gethostname()) == 'fgcz-s-021':
        ...
        BB = BioBeamer(
            ...
            log_file="/home/cpanse/__git_clones/BioBeamer/fgcz_biobeamer.log"
        )
